...
describe('post Client', () => {
    ...
    test('should create a client and get an error because the client has already been created for that user', async () => {
        ...
        const responseFail1 = await request(app)
            .post('/practica/client')
            .auth(tokenUserPrueba, { type: 'bearer' })
            .send(clientPrueba)
            .set('Accept', 'application/json')
...
describe('restore a client', () => {
    test('should get an error "NOT_SESSION"', async () => {
        const response = await request(app)
            .post(`/practica/client/restore/${clientDeletedA._id.toString()}`)
            .expect(401)
    })
    test('should get an error CLIENT NOT FOUND', async () => {
        const response1 = await request(app)
            .post("/practica/client/restore/123456789012345678901234")
            .auth(tokenUserPrueba, { type: 'bearer' })
            .expect(404)
        const response2 = await request(app)
            .post("/practica/client/restore/1")
            .auth(tokenUserPrueba, { type: 'bearer' })
            .expect(404)
    })
    test('should restore a client', async () => {
        const response = await request(app)
            .post(`/practica/client/restore/${clientDeletedA._id.toString()}`)
            .auth(tokenUserPrueba, { type: 'bearer' })
            .expect(200)
